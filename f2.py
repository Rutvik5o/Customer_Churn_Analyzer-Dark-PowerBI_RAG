# app.py
# Customer Churn Analyzer — PowerBI-style Dark Dashboard + RAG Q&A with Perplexity Sonar
...
from openai import OpenAI  # Perplexity-compatible client
    ...
    <div class="header-card">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
        <div>
          ...
          <div class="header-sub" style="font-size:0.95rem;margin-top:6px;">Interactive • Visuals • Perplexity Sonar RAG (optional)</div>
        </div>
...
with col_left:
    ...
    st.markdown(
        '<div class="small-muted">CSV should include customerID,tenure,MonthlyCharges,TotalCharges,Contract,InternetService,PaymentMethod,Churn (map in sidebar)</div>',
        unsafe_allow_html=True
    )
with col_right:
    st.markdown(
        '<div class="info-card"><strong>⚡ Quick Tips</strong><br>'
        '• Ensure Tenure & MonthlyCharges are numeric<br>'
        '• Churn: Yes/No or 1/0<br>'
        '• Optional: set PERPLEXITY_API_KEY in Streamlit secrets to enable Sonar answers</div>',
        unsafe_allow_html=True
    )
...
def normalize_churn(series):
    ...
    return s.map({
        'yes': 1, 'y': 1, 'true': 1, '1': 1, 't': 1, 'churn': 1,
        'no': 0, 'n': 0, 'false': 0, '0': 0, 'stay': 0, 'retained': 0
    }).astype(float)
...
def dark_plotly_layout(fig, height=420, showlegend=True, rotate_x=False):
    ...
    try:
        if rotate_x:
            fig.update_xaxes(
                tickangle=-45, automargin=True, showgrid=False, zeroline=False,
                showline=True, linecolor='rgba(255,255,255,0.06)', tickfont=dict(color='#d0e8e2')
            )
        else:
            fig.update_xaxes(
        ...
        fig.update_yaxes(
            automargin=True, showgrid=True, gridcolor='rgba(255,255,255,0.03)',
            zeroline=False, showline=True, linecolor='rgba(255,255,255,0.06)',
            tickfont=dict(color='#d0e8e2')
        )
    except Exception:
        pass
    return fig
...
# ------------------------- Perplexity Sonar Helper -----------------------
def call_perplexity_generate(context_text: str, user_question: str):
    """Call Perplexity Sonar to answer based only on provided context."""
    api_key = st.secrets.get("PERPLEXITY_API_KEY")
    if not api_key:
        return "Perplexity API key not found in secrets. Please set PERPLEXITY_API_KEY."
...
"""
    ...
    try:
        resp = client.chat.completions.create(
            model="sonar",  # or "sonar-pro" if desired
            messages=[
                {"role": "system", "content": "You are a helpful churn analytics assistant."},
...
if run_analysis:
    ...
    st.markdown(
        f'<div class="info-card"><strong>📋 Filtered Dataset</strong> • {len(df2)} rows</div>',
        unsafe_allow_html=True,
    )
    ...
    with k1:
        k1_html = (
            '<div class="metric-card"><div class="metric-sub">👥 Total Customers</div>'
            f'<div class="metric-value">{total_customers:,}</div></div>'
        )
        st.markdown(k1_html, unsafe_allow_html=True)
    with k2:
        if churn_rate is not None:
            st.markdown(
                '<div class="metric-card"><div class="metric-sub">🔴 Churn Rate</div>'
                f'<div class="metric-value">{churn_rate:.1%}</div></div>',
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
    ...
    with k3:
        avg_tenure_fmt = (
            "N/A" if avg_tenure is None or (isinstance(avg_tenure, float) and np.isnan(avg_tenure))
            else f"{avg_tenure:.1f}"
        )
        st.markdown(
            '<div class="metric-card"><div class="metric-sub">📅 Avg Tenure</div>'
            f'<div class="metric-value">{avg_tenure_fmt}</div></div>',
            unsafe_allow_html=True,
        )
    with k4:
        avg_monthly = (
        ...
        avg_monthly_fmt = (
            "N/A" if avg_monthly is None or (isinstance(avg_monthly, float) and np.isnan(avg_monthly))
            else f"{avg_monthly:.2f}"
        )
        st.markdown(
            '<div class="metric-card"><div class="metric-sub">💳 Avg Monthly</div>'
            f'<div class="metric-value">{avg_monthly_fmt}</div></div>',
            unsafe_allow_html=True,
        )
    ...
    with left_col:
        st.markdown(
            '<div class="chart-card"><strong>📈 Retention (Tenure Survival)</strong></div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="chart-card" style="margin-top:12px;"><strong>📦 Monthly Charges by Churn (Box)</strong></div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            '<div class="chart-card" style="margin-top:12px;"><strong>📊 Churn Rate by Contract Type</strong></div>',
            unsafe_allow_html=True,
        )
        if "Contract" in df2.columns and churn_key:
            tmp = df2[["Contract", churn_key]].dropna()
            if tmp.shape[0] > 0:
                agg = tmp.groupby("Contract").agg(
                    total=("Contract", "count"),
                    churned=(churn_key, "sum"),
                ).reset_index()
                ...
                fig = px.bar(
                    ...
                    x="Contract",
                    y="churn_rate",
                    text=agg["churn_rate"].apply(lambda x: "{:.0%}".format(x)),
    ...
    with right_col:
        st.markdown(
            '<div class="chart-card"><strong>🍰 Churn Distribution</strong></div>',
            unsafe_allow_html=True,
        )
        if churn_key:
        ...
        st.markdown(
            '<div class="chart-card" style="margin-top:12px;"><strong>🔍 Top Drivers (Service Flags)</strong></div>',
            unsafe_allow_html=True,
        )
        service_cols = [
            c
        ...
        if service_cols and churn_key:
            ...
            for c in service_cols:
                ...
                if tmp.shape[0] == 0:
                    continue
                mapped = (
                    tmp[c].astype(str).str.strip().str.lower().replace(
                ...
                if tmp_local.shape[0] == 0:
                    continue
                agg = tmp_local.groupby("_flag").agg(
                    total=("_flag", "count"),
                    churned=(churn_key, "sum"),
                ).reset_index()
                if 1 in agg["_flag"].values:
                    try:
                        churn_rate_flag = float(
                            agg.loc[agg["_flag"] == 1, "churned"].values[0]
                        ) / float(
                            agg.loc[agg["_flag"] == 1, "total"].values[0]
                        )
                    except Exception:
                        churn_rate_flag = 0.0
                else:
            ...
            if not feat_df.empty:
                fig = px.bar(
                    ...
                    y="feature",
                    orientation="h",
                    text=feat_df["churn_rate_if_yes"].apply(lambda x: "{:.0%}".format(x)),
                    color_discrete_sequence=PALETTE,
                )
    ...
    with c1:
        st.markdown(
            '<div class="chart-card"><strong>💳 Churn by Payment Method</strong></div>',
            unsafe_allow_html=True,
        )
        if "PaymentMethod" in df2.columns and churn_key:
            tmp = df2[["PaymentMethod", churn_key]].dropna()
            if tmp.shape[0] > 0:
                agg = tmp.groupby("PaymentMethod").agg(
    ...
    with c2:
        st.markdown(
            '<div class="chart-card"><strong>🌐 Churn by Internet Service</strong></div>',
            unsafe_allow_html=True,
        )
        if "InternetService" in df2.columns and churn_key:
            tmp = df2[["InternetService", churn_key]].dropna()
            if tmp.shape[0] > 0:
                agg = tmp.groupby("InternetService").agg(
                    total=("InternetService", "count"),
                    churned=(churn_key, "sum"),
                ).reset_index()
                agg["churn_rate"] = agg["churned"] / agg["total"]
                agg = agg.sort_values("churn_rate", ascending=False)
    ...
    st.markdown(
        '<div class="chart-card"><strong>🧭 Numeric Correlation</strong></div>',
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        summary_html += (
            f'<div>Overall churn rate: {("{:.1%}".format(churn_pct) if churn_pct is not None else "N/A")}</div>'
        )
...
st.markdown("---")
st.markdown(
    '<div class="chart-card"><strong>🔎 RAG Q&A (ask about your dataset)</strong></div>',
    unsafe_allow_html=True,
)
...
if st.session_state.get("_rag_source_sig") != _source_sig:
    ...
    st.session_state["rag_passages"] = build_corpus_from_df_sampled(
        _df_for_rag,
        max_rows=MAX_RAG_ROWS,
        max_chars=MAX_PASSAGE_CHARS,
    )
    st.session_state["rag_vect"], st.session_state["rag_mat"] = build_tfidf_index_safe(
        st.session_state.get("rag_passages", []),
...
st.caption(
    f"Indexing: up to {MAX_RAG_ROWS} rows sampled • truncating long rows • TF-IDF max features: {TFIDF_MAX_FEATURES}"
)

user_question = st.text_input(
    "Ask a question about the dataset (e.g., 'Which contract has highest churn?')",
    key="rag_ui_question",
)
...
perplexity_checkbox = st.checkbox(
    "Use Perplexity Sonar (if PERPLEXITY_API_KEY is set)",
    value=False,
    key="rag_ui_perplexity",
)
...
if st.button("Run RAG", key="run_rag_ui"):
    if not user_question or not user_question.strip():
        ...
        st.session_state["rag_vect"] is None
        or st.session_state["rag_mat"] is None
        or len(st.session_state["rag_passages"]) == 0
    ):
        st.error(
            "Retriever not ready or no passages indexed. Try re-running analysis or reduce MAX_RAG_ROWS."
        )
    else:
        try:
            with st.spinner("Retrieving relevant passages..."):
                retrieved = retrieve_top_k_from_index(
                    user_question,
                    st.session_state["rag_vect"],
                    st.session_state["rag_mat"],
                    st.session_state["rag_passages"],
                    k=top_k,
                )
            if not retrieved:
                ...
                llm_answer = None
                if perplexity_checkbox:
                    try:
                        st.info("Calling Perplexity Sonar (this may take several seconds)...")
                        llm_answer = call_perplexity_generate(context_text, user_question)
                    except Exception as e:
                        st.error(f"Perplexity call failed: {e}")
                        llm_answer = None

                if llm_answer:
                    st.subheader("Answer from Perplexity Sonar")
                    st.write(llm_answer)
                else:
                    st.subheader("Extractive fallback answer")
                    summary = extractive_summary_from_passages(
                        [p for _, p, _ in retrieved],
                        user_question,
                        max_sentences=4,
                    )
                    if summary:
